...
export function CostCalculator() {
  ...
  useEffect(() => {
    if (!watchedMachines || allMachines.length === 0) {
      return;
    }

    const getTransportCalculationData = () => {
        if (!watchedMachines || watchedMachines.length === 0) return null;
    ...
    setTransportData(getTransportCalculationData());
  }, [watchedMachines, allMachines]);
